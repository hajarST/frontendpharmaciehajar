...
const PharmacyFinder = () => {
  ...
  return (
    <div style={{ width: "100%" }}>
    <div>
      <h1 className="mt-4 mb-4 text-center"  style={{color: "blue"}}>Chercher votre pharmacie à proximité dans GeoPharma</h1>
      <hr />

      <div>
        <label htmlFor="city-select">Sélectionner une ville:</label>
        ...
        <div>
          ...
          {selectedZone && (
            <div>
              ...
              <div>
                <label htmlFor="garde-select">Sélectionner le type de garde</label>
                    ...
                    <div>
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: "20px" }}>
                        {pharmacies.map(pharmacy => (
                          <Card key={pharmacy.id} sx={{ maxWidth: 300 }}>
                            <CardMedia
                              component="img"
                              alt={pharmacy.name}
                              height="100px"
                            ...
                            Nom du pharmacie : {pharmacy.name}
                                </Typography>
                                <hr className="my-4" />
                                ...
                                <Typography variant="body2" color="text.primary">
                                  Addresse: {pharmacy.address}
                                </Typography>
                            </CardContent>
                            <CardActions>
                              <Button onClick={() => setModalIsOpen(true)} size="small" style={{ backgroundColor: "green", color: "white" }}>
                                Voir détails 
                              </Button>
                              <ReactModal isOpen={modalIsOpen}>
                                <h2 >Détails de {pharmacy.name}</h2>
                                
                                <p>{pharmacy.name} est une pharmacie qui se trouve à {pharmacy.address}</p>
                                
                                <button style={{ backgroundColor: "red", color: "white" }} onClick={() => setModalIsOpen(false)}>Fermer</button>
                              </ReactModal>
                            </CardActions>
                          </Card>
                        ))}
